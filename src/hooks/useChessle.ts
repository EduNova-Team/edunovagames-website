 ...
 * Green first, then yellow, per the spec.
 */
...
export function useChessle() {
  const [opening, setOpening] = useState<Opening>(() => pickRandomOpening());
  ...
  const submitGuess = useCallback(() => {
    if (phase !== "playing") return;
    if (currentMoveIndex < HALF_MOVES_PER_GUESS) return; // row not full
    ...
    if (isWin) {
      ...
      // Reset chess board to starting position for the new guess
      setChess(new Chess());
    }
  ...
  const undoMove = useCallback(() => {
    ...
    setCurrentMoveIndex((i) => i - 1);
  }, [phase, currentMoveIndex, currentGuessIndex, chess]);
